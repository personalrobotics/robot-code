...
- **Do not include `Co-Authored-By` lines in commit messages.** Commits should be authored as the user, not Claude.
- **Use GitHub Issues for tracking work.** Log follow-ups, discovered bugs, and improvement ideas as GitHub issues (via `gh issue create`) rather than local TODO files. Use the templates in `.github/ISSUE_TEMPLATE/`:
  - `bug_report.md` — for bugs and unexpected behavior.
  - `improvement.md` — for speed, robustness, scalability, or code quality improvements.
  - `task.md` — for general work items and feature requests.
...
- **Testing**: pytest, run with `uv run pytest tests/ -v`
- **Commits**: Co-authored with Claude, use heredoc for commit messages
- **Issues**: When filing bugs/enhancements, use detailed templates
  - Include severity, file locations, code snippets, suggested fixes, and task checklists
  - Group related bugs into single issues when they share a theme


- **CLAUDE.md updates**: Ask permission before adding new sections or patterns to this file.
